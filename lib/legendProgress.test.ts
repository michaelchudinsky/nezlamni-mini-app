...
test("30 non-consecutive active days unlock stork, not wolf", () => {
  const logs = Array.from({ length: 30 }, (_, index) => ({ task_code: "water_wakeup", event_day: day(index * 2), points: 1 }));
...
test("100 Food Quest unlock fox", () => assert.equal(result(logsFor(100, ["bonus_day_1_daily"])).get("fox")?.unlocked, true));
test("100 Sport Challenge unlock horse", () => assert.equal(result(logsFor(100, ["bonus_day_1_challenge"])).get("horse")?.unlocked, true));
test("30 perfect days unlock deer", () => assert.equal(result(logsFor(30, baseCodes)).get("deer")?.unlocked, true));
test("60 perfect days unlock lynx", () => assert.equal(result(logsFor(60, baseCodes)).get("lynx")?.unlocked, true));
...
test("250 Sport Challenge unlock bear", () => assert.equal(result(logsFor(250, ["bonus_day_1_challenge"])).get("bear")?.unlocked, true));
...
test("180 active days unlock owl", () => assert.equal(result(logsFor(180, ["water_wakeup"])).get("owl")?.unlocked, true));
test("30 consecutive Food Quest unlock swan", () => assert.equal(result(logsFor(30, ["bonus_day_1_daily"])).get("swan")?.unlocked, true));
